...
def Plot2DMassRes1b(
    ...
    for xbin in range(1, hist_denum.GetNbinsX() + 1):
        if (
            int(hist_denum.GetXaxis().GetBinCenter(xbin))
            == global_cfg_dict["mass_cut_limits"]["bb_m_vis"]["other"][0]
        ):
            x1 = hist_denum.GetXaxis().GetBinLowEdge(xbin)
        if (
            int(hist_denum.GetXaxis().GetBinLowEdge(xbin))
            == global_cfg_dict["mass_cut_limits"]["bb_m_vis"]["other"][0]
        ):
            x1 = hist_denum.GetXaxis().GetBinLowEdge(xbin)
        if (
            int(hist_denum.GetXaxis().GetBinCenter(xbin))
            == global_cfg_dict["mass_cut_limits"]["bb_m_vis"]["other"][1]
        ):
            x2 = hist_denum.GetXaxis().GetBinLowEdge(xbin)
        if (
            int(hist_denum.GetXaxis().GetBinUpEdge(xbin))
            == global_cfg_dict["mass_cut_limits"]["bb_m_vis"]["other"][1]
        ):
    ...
    for ybin in range(0, hist_denum.GetNbinsY() + 1):
        if (
            hist_denum.GetYaxis().GetBinCenter(ybin)
            == global_cfg_dict["mass_cut_limits"]["tautau_m_vis"][0]
        ):
            y1 = hist_denum.GetYaxis().GetBinLowEdge(ybin)
        if (
            hist_denum.GetYaxis().GetBinLowEdge(ybin)
            == global_cfg_dict["mass_cut_limits"]["tautau_m_vis"][0]
        ):
            y1 = hist_denum.GetYaxis().GetBinLowEdge(ybin)
        if (
            hist_denum.GetYaxis().GetBinCenter(ybin)
            == global_cfg_dict["mass_cut_limits"]["tautau_m_vis"][1]
        ):
            y2 = hist_denum.GetYaxis().GetBinLowEdge(ybin)
        if (
            hist_denum.GetYaxis().GetBinUpEdge(ybin)
            == global_cfg_dict["mass_cut_limits"]["tautau_m_vis"][1]
        ):
    ...
    if resonance != "":
        outFileName = (
            f"Studies/MassCuts/output/Run2_{year}/res1b/{channel}_{resonance}_M-{mass}"
        )
    print(outFileName)
    plot_2D_histogram(
        hist_denum,
...
def Plot2DMassRes2b(
    ...
    if resonance != "":
        outFileName = (
            f"Studies/MassCuts/output/Run2_{year}/res2b/{channel}_{resonance}_M-{mass}"
        )
    print(outFileName)
    plot_2D_histogram(
        hist_denum,
...
def Plot2DMassboosted(
    ...
    for xbin in range(0, hist_denum.GetNbinsX() + 1):
        if (
            hist_denum.GetXaxis().GetBinCenter(xbin)
            == global_cfg_dict["mass_cut_limits"]["bb_m_vis"]["boosted"][0]
        ):
            x1 = hist_denum.GetXaxis().GetBinLowEdge(xbin)
        if (
            hist_denum.GetXaxis().GetBinLowEdge(xbin)
            == global_cfg_dict["mass_cut_limits"]["bb_m_vis"]["boosted"][0]
        ):
            x1 = hist_denum.GetXaxis().GetBinLowEdge(xbin)
        if (
            hist_denum.GetXaxis().GetBinCenter(xbin)
            == global_cfg_dict["mass_cut_limits"]["bb_m_vis"]["boosted"][1]
        ):
            x2 = hist_denum.GetXaxis().GetBinLowEdge(xbin)
        if (
            hist_denum.GetXaxis().GetBinUpEdge(xbin)
            == global_cfg_dict["mass_cut_limits"]["bb_m_vis"]["boosted"][1]
        ):
            x2 = hist_denum.GetXaxis().GetBinUpEdge(xbin)
...
if __name__ == "__main__":
    ...
    inFiles = Utilities.ListToVector(
        [
            ...
            f"/eos/user/a/aciocci/HHbbTauTauRes/anaTuples/v13_deepTau2p1_HTT/SC/Run2_{args.year}/GluGluToBulkGravitonToHHTo2B2Tau_M-900/nanoHTT_0.root",
            f"/eos/user/a/aciocci/HHbbTauTauRes/anaTuples/v13_deepTau2p1_HTT/SC/Run2_{args.year}/GluGluToRadionToHHTo2B2Tau_M-1000/nanoHTT_0.root",
    ...
    for mass in masses_list:
        if args.res == "radion":
            inFiles.append(
                f"/eos/user/a/aciocci/HHbbTauTauRes/anaTuples/v13_deepTau2p1_HTT/SC/Run2_{args.year}/GluGluToBulkGravitonToHHTo2B2Tau_M-{mass}/nanoHTT_0.root"
            )
        elif args.res == "graviton":
            inFiles.append(
                f"/eos/user/a/aciocci/HHbbTauTauRes/anaTuples/v13_deepTau2p1_HTT/SC/Run2_{args.year}/GluGluToRadionToHHTo2B2Tau_M-{mass}/nanoHTT_0.root"
            )
        else:
            inFiles.append(
                f"/eos/user/a/aciocci/HHbbTauTauRes/anaTuples/v13_deepTau2p1_HTT/SC/Run2_{args.year}/GluGluToBulkGravitonToHHTo2B2Tau_M-{mass}/nanoHTT_0.root"
    ...
    dfWrapped = PrepareDfForHistograms(
        DataFrameBuilderForHistograms(df_initial, global_cfg_dict, f"Run2_{args.year}")
    )
