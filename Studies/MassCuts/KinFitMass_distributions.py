...
def GetCorrectBinning():
    ...
    bins = [centri[i] for i in range(len(centri) - 1)] + [
        centri[-1] + 1
    ]  # Aggiungi un valore massimo per il bin finale
    ...
    limits.append(
        centri[-1] + (centri[-1] - centri[-2]) / 2
    )  # Limite superiore dell'ultimo bin
...
def PlotMass(
    ...
    total_weight_expression = GetWeight(
        channel, cat, global_cfg_dict["boosted_categories"]
    )  # if sample_type!='data' else "1"
    ...
    hist_kinFit_m = (
        df.Filter(filter_str)
        .Define("final_weight", total_weight_expression)
        .Histo1D(GetModel1D(kinFit_bins), "kinFit_m", "final_weight")
        .GetValue()
    )
    hist_bbttmass = (
        df.Filter(filter_str)
        .Define("final_weight", total_weight_expression)
        .Histo1D(GetModel1D(bbtautau_mass_bins), "bbtautau_mass", "final_weight")
        .GetValue()
    )
    ...
    plot_1D_histogram(
        [hist_kinFit_m, hist_bbttmass],
        ["kinFit_mass", "bbtautau_mass"],
        bbtautau_mass_bins,
        f"{channel} Res1b",
        [],
        outFileName,
...
if __name__ == "__main__":
    ...
    for mass in masses_list:
        ...
        if args.res == "radion":
            inCacheFiles.append(
                f"/eos/user/v/vdamante/HH_bbtautau_resonant_Run2/anaCacheTuples/Run2_{args.year}/GluGluToBulkGravitonToHHTo2B2Tau_M-{mass}/v13_deepTau2p1_HTT/SC/nanoHTT_0.root"
            )
        elif args.res == "graviton":
            inCacheFiles.append(
                f"/eos/user/v/vdamante/HH_bbtautau_resonant_Run2/anaCacheTuples/Run2_{args.year}/GluGluToRadionToHHTo2B2Tau_M-{mass}/v13_deepTau2p1_HTT/SC/nanoHTT_0.root"
            )
        else:
            inCacheFiles.append(
                f"/eos/user/v/vdamante/HH_bbtautau_resonant_Run2/anaCacheTuples/Run2_{args.year}/GluGluToBulkGravitonToHHTo2B2Tau_M-{mass}/v13_deepTau2p1_HTT/SC/nanoHTT_0.root"
    ...
    dfWrapped_central = DataFrameBuilderForHistograms(
        df_initial,
        global_cfg_dict,
        period=f"Run2_{args.year}",
        region="SR",
        isData=False,
    )

    dfWrapped_cache_central = DataFrameBuilderForHistograms(
        df_cache_initial,
        global_cfg_dict,
        period=f"Run2_{args.year}",
        region="SR",
