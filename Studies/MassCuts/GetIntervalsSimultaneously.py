...
def GetMassCut(
    dfWrapped, global_cfg_dict, channels=[], categories=[], quantile_max=0.99
):
    for cat in categories:
        ...
        if cat != "boosted":
            ...
            df_cat = df_cat.Define(
                "FatJet_atLeast2BHadron", "SelectedFatJet_nBHadrons>=2"
            ).Filter("SelectedFatJet_p4[FatJet_atLeast2BHadron].size()>0")
            ...
            if cat == "boosted":
                ...
                for mass_boosted in masses_boosted:
                    ...
                    for bb_mass_step in [
                        ...
                        700,
                        800,
                    ]:  # [200, 205, 210, 215, 220, 225, 230, 235, 240, 245, 250, 255, 260, 265, 270, 275, 280, 285, 290, 295,300, 310, 320, 330, 340, 350, 360, 380, 390, 400, 410, 420, 430, 440, 450, 460, 470, 480, 490, 500, 600, 700, 800]:
                        mask_tautau_up = (
                            np_dict_ch["tautau_m_vis"] < upper_cut_tt_boosted
                        )
                        mask_tautau_low = (
                            np_dict_ch["tautau_m_vis"] > lower_cut_tt_boosted
                        )
                        ...
                        combined_mask = (
                            mask_tautau_up & mask_tautau_low & mask_bb_up & mask_bb_low
                        )
                        np_dict_ch_uppercut = {
                        ...
                        np_array_mass_tt_ch_uppercut = np_dict_ch_uppercut[
                            "tautau_m_vis"
                        ]
                        ...
                        if (
                            percentage > quantile_max - 0.1
                            and percentage < quantile_max + 0.1
                        ):
                ...
                for tt_mass_step in [
                    ...
                    for bb_mass_step in [
                        270
                    ]:  # [180, 185, 190, 195, 200, 205, 210, 215, 220, 225, 230, 235, 240, 245, 250, 255, 260, 265, 270, 275, 280, 285, 290, 295, 300]:
                        ...
                        if (
                            ...
                            print(f"for category {cat} and channel {channel}")
                            print(
                                f"condition: tautau_m_vis < {tt_mass_step} && bb_m_vis < {bb_mass_step}"
                            )
                            print(
                                f"bb_mass_step = {bb_mass_step}, tt_mass_step= {tt_mass_step}"
                            )
                            print(
                                f"len_tt_upper = {len_tt_upper}, len_bb_upper= {len_bb_upper}"
                            )
                print()
        print()
    print()
