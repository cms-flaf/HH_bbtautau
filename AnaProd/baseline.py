...
def PassGenAcceptance(df):
    ...
    return df.Filter(
        "PassGenAcceptance(*genHttCandidate)", "genHttCandidate Acceptance"
    )
...
def GenJetSelection(df):
    ...
    df = df.Define(
        "GenJetAK8_B1",
        "GenJetAK8_pt > 170 && abs(GenJetAK8_eta) < 2.5 && GenJetAK8_Hbb",
    )
    return df.Filter(
        "GenJet_idx[GenJet_B1].size()==2 || (GenJetAK8_idx[GenJetAK8_B1].size()==1 && genHbb_isBoosted)",
        "(One)Two b-parton (Fat)jets at least",
    )
...
def GenJetHttOverlapRemoval(df):
    for var in ["GenJet", "GenJetAK8"]:
        df = df.Define(
            f"{var}_B2",
            f"RemoveOverlaps({var}_p4, {var}_B1,{{{{genHttCandidate->leg_p4[0], genHttCandidate->leg_p4[1]}},}}, 2, 0.5)",
        )
    return df.Filter(
        "GenJet_idx[GenJet_B2].size()==2 || (GenJetAK8_idx[GenJetAK8_B2].size()==1 && genHbb_isBoosted)",
        "No overlap between genJets and genHttCandidates",
    )
...
def RecoHttCandidateSelection(df, config):
    ...
    df = (
        df.Define("Electron_iso", "Electron_pfRelIso03_all")
        .Define("Muon_iso", "Muon_pfRelIso04_all")
        .Define(
            "Tau_iso",
            f"""-Tau_rawDeepTau{deepTauVersions[config["deepTauVersion"]]}v{config["deepTauVersion"]}VSjet""",
        )
    )
    ...
    df = df.Define(
        "Electron_B2_eMu_1",
        f"Electron_B0 && (Electron_mvaIso_WP80 || (Electron_mvaNoIso_WP80 && Electron_pfRelIso03_all < 0.3)) ",
    )
    #  Electron_mvaNoIso_WP80 && Electron_pfRelIso03_all < 0.3
    df = df.Define(
        "Muon_B2_eMu_2",
    ...
    df = df.Define(
        "Electron_B2_eE_2",
        f""" Electron_B0 && (Electron_mvaNoIso_WP80 && Electron_pfRelIso03_all < 0.3) """,
    )
...
def RecoJetSelection(df, era):
    jet_puID_cut = (
        "&& (Jet_puId>0 || v_ops::pt(Jet_p4)>50)" if era.startswith("Run2") else ""
    )
    df = df.Define(
        "Jet_bIncl",
    ...
    df = df.Define(
        "Jet_bCand",
        "RemoveOverlaps(Jet_p4, Jet_bIncl,{{HttCandidate.leg_p4[0], HttCandidate.leg_p4[1]},}, 2, 0.5)",
    )
    df = df.Define(
        "FatJet_bbCand",
        "RemoveOverlaps(FatJet_p4, FatJet_bbIncl, {{HttCandidate.leg_p4[0], HttCandidate.leg_p4[1]},}, 2, 0.5)",
    )
    return df
...
def ExtraRecoJetSelection(df, era):
    ...
    df = df.Define(
        ...
        "if(Hbb_isValid){return std::vector<RVecLV>({{HttCandidate.leg_p4[0], HttCandidate.leg_p4[1],HbbCandidate->leg_p4[0],HbbCandidate->leg_p4[1]}}); } return std::vector<RVecLV>({{HttCandidate.leg_p4[0], HttCandidate.leg_p4[1]}})",
    )
    df = df.Define(
        f"ExtraJet_B1",
        """ RemoveOverlaps(Jet_p4, ExtraJet_B0,ObjectsToRemoveOverlap, 2, 0.5)""",
    )
    return df
...
def GenRecoJetMatching(df):
    df = df.Define(
        "Jet_genJetIdx_matched",
        "GenRecoJetMatching(event,Jet_idx, GenJet_idx, Jet_bCand, GenJet_B2, GenJet_p4, Jet_p4 , 0.3)",
    )
    df = df.Define("Jet_genMatched", "Jet_genJetIdx_matched>=0")
    return df.Filter(
        "Jet_genJetIdx_matched[Jet_genMatched].size()>=2",
        "Two different gen-reco jet matches at least",
    )
...
def VBFJetSelection(df):
    ...
    df = df.Define(
        "VBFJet_B0", "VBFJet_B0_base && !(v_ops::pt(Jet_p4) < 50)"
    )  # && abs(v_ops::eta(Jet_p4)) > 2.5 && abs(v_ops::eta(Jet_p4)) < 3.0)")
    df = df.Define(
