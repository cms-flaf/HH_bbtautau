...
namespace recoil_boson {
    ...
    inline bool PassRecoilGenParticleSelection(const int pdgId,
                                               const int status,
                                               const unsigned short statusFlags,
                                               const bool includeNeutrinos) {
        // Required inputs (slide 32): https://indico.cern.ch/event/1583951/contributions/6751916/attachments/3159171/5612627/HLepRare_25.10.22.pdf
        ...
        if (fromHardProcess) {
            if (isChargedLep)
                return true;
            if (includeNeutrinos && isNeutrino)
                return true;
        }

        if (isDirectHardProcessTauDecayProduct) {
            if (!includeNeutrinos && isNeutrino)
                return false;  // skip neutrinos from tau decays if not including neutrinos.
            return true;
        }
    ...
    inline LorentzVectorM GetGenBosonP4(const RVecF& pt,
                                        const RVecF& eta,
                                        const RVecF& phi,
                                        const RVecF& mass,
                                        const RVecI& pdgId,
    ...
    inline LorentzVectorM GetGenBosonVisP4(const RVecF& pt,
                                           const RVecF& eta,
                                           const RVecF& phi,
                                           const RVecF& mass,
                                           const RVecI& pdgId,
        ...
        LorentzVectorM p4(0., 0., 0., 0.);
        for (std::size_t i = 0; i < n; ++i) {
            if (!PassRecoilGenParticleSelection(pdgId[i], status[i], statusFlags[i], false))
                continue;
            p4 += LorentzVectorM(pt[i], eta[i], phi[i], mass[i]);
        }
